...
def tokenize_fsm(expr):
    ...
    while td < stack:
        """Проходимся по элементам нашего выражения и проверяем, к чему элемент
        относится(число,пробел,знак операции), пока наша переменная не дойдет
        до конца выражения."""
        char = expr[td]
        if char == " ":
            td += 1
            ...
            """Проверяем, если попалось число.

            Используем флаг для отображения того, что уже встретили число а так
            же ведем счетчик точек в числе
            """
            ...
            while td < stack and (expr[td].isdigit() or expr[td] == "."):
                """Собираем число.

                Проверяем количетсво точек в текущем выражении
                """
                if expr[td] == ".":
                    dot_count += 1
                    if dot_count > 1:
            ...
            if not has_digit or tek_expr.endswith("."):
                """Если же мы не встретили число или число заканчивается
                точкой, то выводим ошибку."""
                raise InvalidNumberInputError(f"Некорректное число: '{tek_expr}'")
            try:
            ...
            """Проходимся по знакам операций, флаг используем для отметки
            добавления знака операции."""
            Flag = False
            if td + 1 < stack:
                """Проверяем так же, существует ли символ после знака операции,
                тк можем выйти за длину строки."""
            ...
            if not Flag and char == "+" and (td == 0 or expr[td - 1] in "(+-*/% "):
                ...
                if td + 1 < stack and (expr[td + 1].isdigit() or expr[td + 1] == "."):
            ...
            if not Flag and char == "-" and (td == 0 or expr[td - 1] in "(+-*/% "):
                ...
                if td + 1 < stack and (expr[td + 1].isdigit() or expr[td + 1] == "."):
                    """Сначала будем работать с числом после унарного минуса, а
                    затем уже добавим и сам минус."""
                    td += 1
                    ...
                    continue
            if not Flag:
                """Если ничего не добавили, то вносим любой другой опреатор,
                который не проверяли выше."""
...
def is_valid_opn(tokens):
    ...
    for token_type, znach in tokens:
        ...
        if token_type == "NUMBER":
            operand_count += 1
            """При встрече числа, добавляем его в количество общего числа
            знаков."""
        elif token_type == "OPERATOR":
            if znach in {"+", "-", "*", "/", "**", "//", "%"}:
                """При встрече знаков операций, проверяем, сколько было уже
                операндов."""
                if operand_count < 2:
                    return False
                operand_count -= 1
...
def remove_parentheses(tokens, start=0):
    ...
    while i < len(tokens):
        ...
        if znach == "(":
            """Обрабатываем содержимое скобок, запускаем рекурсию, чтобы
            проверить корректность выражения внутри."""
            inner_result, new_i = remove_parentheses(tokens, i + 1)

            """Проверяем корректность выражения,
             выводим ошибку, если некорректно """
            if not is_valid_opn(inner_result):
                raise RPNExpressionError("Некорректное выражение внутри скобок")
            ...
            """Если текущий элемент не скобка, то добавляем его в рещультат и
            смещаемся вправо."""
...
def opn(s):
        ...
        if symb in operator:
            ...
            if symb == "~":
                ...
                try:
                    ...
                    raise RPNExpressionError("Некорректное выражение с унарным минусом")
                ...
                try:
                    ...
                    if symb == "+":
                        ...
                        if b == 0:
                            raise ZeroDivisionMathError("На ноль делить нельзя!!!")
                        tek_znach = a // b
                    elif symb == "%":
                        if b == 0:
                            raise ZeroDivisionMathError("На ноль делить нельзя!!!")
                    ...
                    raise RPNExpressionError("Некорректные операнды")
    ...
    if len(stack) == 0:
        ...
        raise TooManyOperandsError(f"В выражении остались лишние операнды: {stack}")
